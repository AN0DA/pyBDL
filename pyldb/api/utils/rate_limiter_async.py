...
from pyldb.api.exceptions import RateLimitDelayExceeded, RateLimitError
...
class AsyncRateLimiter:
    ...
    def __init__(
        ...
        self.lock = asyncio.Lock()  # Single lock for all periods
    ...
    def get_remaining_quota(self) -> dict[int, int]:
        ...
        # For async usage, use await get_remaining_quota_async()
        for period in self.quotas:
            q = self.calls[period]
            limit = self._get_limit(period)
    ...
    async def acquire(self) -> None:
        ...
        - If raise_on_limit=False: Waits until quota available
        ...
        if max_wait > 0:
            ...
            await asyncio.sleep(max_wait)  # Non-blocking sleep
